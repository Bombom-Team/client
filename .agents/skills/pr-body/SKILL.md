...
5. **PR target branch는 항상 `dev`**: 이 레포의 PR은 `dev` 브랜치를 대상으로 만든다. `gh pr create`에 반드시 `--base dev`를 명시한다.
...
현재 브랜치가 `dev`에서 갈라진 이후의 변경을 본다 (이 레포의 PR base는 `dev`):

```bash
git branch --show-current                                     # 브랜치명 (이슈 키 추출용)
git log $(git merge-base HEAD dev)..HEAD --oneline            # 커밋 목록
git log $(git merge-base HEAD dev)..HEAD                       # 커밋 메시지 본문
git diff $(git merge-base HEAD dev)...HEAD --stat             # 파일별 변경량
git diff $(git merge-base HEAD dev)...HEAD                    # 실제 diff
```

브랜치를 `dev`가 아닌 다른 곳에서 분기한 경우 `git log --all --oneline -20`로 기준점을 확인하거나 사용자에게 묻는다.
...
**👀 Review Point** — 리뷰어가 집중할 확인 포인트 (체크박스)

- 리뷰어·자동 리뷰 봇이 **무엇을 중점적으로 봐야 하는지** 구체적으로 적는다.
  "코드 확인", "동작 테스트" 같은 막연한 항목은 리뷰 포인트가 되지 못하므로 금지.
- 다음 관점에서 도출한다:
  - **회귀 위험** — 이 변경으로 깨질 수 있는 기존 동작
  - **사이드이펙트** — 공유 컴포넌트·훅·유틸·타입 변경이 다른 사용처에 미치는 영향
  - **엣지 케이스** — 빈 값/에러/로딩/경계값에서의 동작
  - **디자인·UX QA** — 반응형 등 눈으로 확인이 필요한 것
  - **의도적 트레이드오프** — 리뷰어 판단이 필요한 결정
- 각 항목은 "어디서 / 무엇을 확인"이 드러나게 구체적으로 쓴다.
- 예:
  - `- [ ] shared/Button 변경이 web·admin 사용처에 회귀 없는지 확인`
  - `- [ ] 목록이 빈 배열일 때 EmptyState 정상 노출 확인`
- 안 좋은 예: `- [ ] 코드 확인`, `- [ ] 동작 테스트` (막연함)
...
프로젝트 컨벤션(`AGENTS.md` §12 Git Workflow)에 따라 제목은 다음 형식:

```
[{issue_key}] {type}: {subject}
```

- `issue_key`: 브랜치명·커밋 메시지에서 추출한 `BOM-\d+` (없으면 `[{issue_key}]` prefix 자체를 생략하고 `{type}: {subject}`)
- `type`: 커밋의 주된 type (`feat`, `fix`, `refactor`, `chore`, `docs`, `test`, `style`)
- `subject`: 한국어로 간결하게 (70자 이내)

예: `[BOM-942] feat: 챌린지 랜딩 플로팅 버튼 추가` / 이슈 키 없으면 `feat: 챌린지 랜딩 플로팅 버튼 추가`
...
```
**제목**: [BOM-942] feat: 챌린지 랜딩 플로팅 버튼 추가
...
gh pr create --base dev --title "[BOM-942] feat: 챌린지 랜딩 플로팅 버튼 추가" --body "$(cat <<'EOF'
## 📌 What
...
EOF
)"
...
`--base` 옵션은 **항상 `dev`로 지정한다** — 이 레포의 PR target branch는 `dev`다. `main` 등 다른 base가 필요한 특수한 경우에만 사용자에게 확인한다.
