...
봄봄은 여러 뉴스레터를 한곳에서 읽고, 읽은 내용을 기록하며, 꾸준히 읽는 습관을 만드는 뉴스레터 리딩 플랫폼이에요.
...
읽고 싶은 뉴스레터를 구독해도 메일함에 콘텐츠가 쌓이면 꾸준히 읽기 어려워요. 봄봄은 뉴스레터를 한곳에 모으고, 읽기 기록과 메모를 남기며, 다른 참여자와 함께 읽는 경험을 제공해요.
...
| 아카이빙 | 남긴 하이라이트와 메모를 모아 지식 노트처럼 관리해요 |
...
| --- | --- | --- |
| 뉴스레터가 쌓여만 간다 | 통합 보관 + 읽기 목표 설정 | 읽는 습관 형성 |
| 어디까지 읽었는지 기억나지 않는다 | 읽기 진행률 자동 기록 | 이어서 읽기 |
...
- 구독할 뉴스레터의 정보 페이지에서 **구독하기**를 눌러요.
- 봄봄에서 발급한 구독 이메일 주소를 입력하면 구독이 완료돼요.
- 구독한 뉴스레터는 봄봄 보관함에서 한곳에 모아 읽을 수 있어요.
...
![TanStack Router](https://img.shields.io/badge/TanStack%20Router-%23FF4154.svg?style=for-the-badge&logo=reactrouter&logoColor=white)
...
[백엔드 레포지토리](https://github.com/woowacourse-teams/2025-bom-bom)
