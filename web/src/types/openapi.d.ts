...
export interface paths {
  ...
  '/api/v1/challenges/{challengeId}/reviews/{reviewId}': {
    ...
    /**
     * 리뷰 수정
     * @description 로그인한 사용자가 자신의 챌린지 리뷰를 수정합니다. 코멘트와 비공개 여부를 함께 수정할 수 있습니다. 본인 리뷰만 수정 가능하며, 타인 리뷰에 대한 요청은 정보 누설 방지를 위해 404 로 응답합니다.
     */
  ...
  '/api/v1/challenges/{id}/application': {
    parameters: {
      query?: never;
      header?: never;
      path?: never;
     ...
     */
    post: operations['applyChallenge'];
    /**
     * 챌린지 취소
  ...
  '/api/v1/challenges/{challengeId}/reviews': {
    ...
    /**
     * 리뷰 작성
     * @description 로그인한 사용자가 챌린지 리뷰를 작성합니다. 비공개 여부를 함께 지정할 수 있습니다. 본인이 참여한 챌린지에 대해서만 작성 가능하며, 비참여자 요청은 정보 누설 방지를 위해 404 로 응답합니다. 리뷰는 챌린지 마지막 날(종료일) 부터 작성 가능합니다. 종료일 당일 작성 시 출석 인정, 종료 후 작성은 리뷰만 저장됩니다.
     */
  ...
  '/api/v1/challenges/{id}': {
     ...
     */
    get: operations['getChallengeInfo'];
    put?: never;
    post?: never;
    delete?: never;
    options?: never;
  ...
  '/api/v1/challenges/{challengeId}/reviews/me': {
    ...
    /**
     * 내가 작성한 리뷰 조회
     * @description 로그인한 사용자가 해당 챌린지에 이미 작성한 리뷰가 있는지 확인합니다. 리뷰가 존재하면 200과 함께 리뷰 본문을 반환하고, 없으면 404를 반환합니다.
     */
...
export interface components {
  schemas: {
    ...
    TodayTodoResponse: {
      /** @enum {string} */
      challengeTodoType?: 'READ' | 'COMMENT' | 'MINDSET' | 'REVIEW';
      /** @enum {string} */
...
export interface operations {
  ...
  updateReview: {
    ...
    responses: {
      ...
      /** @description 리뷰를 찾을 수 없음 (미존재 / 경로의 챌린지 불일치 / 타인 리뷰 — IDOR 방어) */
      404: {
        headers: {
          [name: string]: unknown;
        };
  ...
  applyChallenge: {
    ...
    responses: {
      ...
      /** @description 잘못된 요청 (이미 시작된 챌린지, 중복 신청, 구독하지 않은 뉴스레터 등) */
      400: {
        headers: {
          [name: string]: unknown;
        };
  ...
  createReview: {
    ...
    responses: {
      ...
      /** @description 챌린지를 찾을 수 없음 또는 본인이 참여하지 않은 챌린지 */
      404: {
        headers: {
          [name: string]: unknown;
        };
