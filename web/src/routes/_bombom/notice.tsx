...
function NoticePage() {
  ...
  return (
    <Container>
      <TitleWrapper>
        <TitleIconBox>
          <StyledMenuIcon width={14} height={14} color={theme.colors.white} />
        </TitleIconBox>
        <Title>공지사항</Title>
      </TitleWrapper>
      ...
      <ContentWrapper device={device}>
        {sortedContents.map((content) => (
          <Accordion
            ...
            isOpen={openNoticeId === content.noticeId}
            onToggle={() => handleToggle(content.noticeId)}
          >
            {content.content.split('.').map((text) => (
              <AccordionContent key={content.noticeId}>{text}</AccordionContent>
            ))}
          </Accordion>
        ))}
      </ContentWrapper>
