...
- 전체 회원·기간별 가입·일별 가입·오늘 활동은 백엔드에서 `member.role_id = 4`만 제외한다.
- 오늘 활동은 유효 세션 기준이다. 과거 DAU를 복원하거나 정확한 일별 누적으로 표시하지 않는다.
- 오늘 활동의 테스트 계정은 기존 세션 principal과 회원 nickname 연결 기준으로 제외한다. 회원 연결이 없는 세션은 기존 집계 의미를 보존해 포함한다.
...
회원·가입·오늘 활동 구현과 탈퇴 집계의 남은 제한을 구분해 검토한다.
